...
namespace OgreMayaExporter
{
	...
	std::string FxOgreScene::getLightTypeString(FxOgreLightType type)
	{
		...
		std::cout << "Invalid light type detected. Using point light as default.\n";
		return("point");
	}
	...
	MStatus FxOgreScene::writeSceneFile(ParamList &params)
	{
		...
		for( int i = 0; i < m_lights.size(); ++i )
		{
			...
			if(pNodeElement)
			{
				...
				// In Maya, cameras come in facing negative z.  Any rotation will be captured by the node.
		...
		xmlDoc.SaveFile( params.sceneFilename.asChar() );
		return MS::kSuccess;
	}
