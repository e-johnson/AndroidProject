...
namespace OgreMayaExporter
{
	...
	MStatus Skeleton::load(MFnSkinCluster* pSkinCluster,ParamList& params)
	{
		...
		for (int i=0; i<numInfluenceObjs; i++)
		{
			...
			// Doug Perkowski - 5/13/2009
			// .FaceFX exporters allow non-joint objects (transform nodes).  These must be included in the skeleton
			// for all content to work correctly.
			//if (influenceDags[i].hasFn(MFn::kJoint))
			if (influenceDags[i].hasFn(MFn::kTransform))
			{
				//retrieve root joint
				rootDag = jointDag;
				while (jointDag.length()>0)
				{
					...
					if (jointDag.hasFn(MFn::kTransform) && jointDag.length()>0)
						rootDag = jointDag;
				}
	...
	MStatus Skeleton::loadJoint(MDagPath& jointDag,joint* parent,ParamList& params,MFnSkinCluster* pSkinCluster)
	{
		...
		if (jointDag.hasFn(MFn::kTransform))
		{
			MFnTransform jointFn(jointDag);
			// Display info
	...
	MStatus Skeleton::createOgreBones(Ogre::SkeletonPtr pSkeleton,ParamList& params)
	{
		...
		{
			std::cout << "Failure: Skeleton has more than OGRE_MAX_NUM_BONES.  No bones will be exported.\n";
            MGlobal::executeCommand("confirmDialog 	-title \"Error\" -message \"Failure: Skeleton has more than OGRE_MAX_NUM_BONES.  No bones will be exported.\"", true);
			return MS::kFailure;
		}
