...
namespace OgreMayaExporter
{
	...
	void ParamList::parseArgs(const MArgList &args)
	{
		...
		for (unsigned int i = 0; i < args.length(); i++ )
		{
			...
			else if ((MString("-scene") == args.asString(i,&stat)) && (MS::kSuccess == stat))
			{
				exportScene = true;
				sceneFilename = args.asString(++i,&stat);
			}
			else if ((MString("-useBlendshapeDeformerName") == args.asString(i,&stat)) && (MS::kSuccess == stat))
			{
				useBlendshapeDeformerName = true;
			}
