...
namespace OgreMayaExporter
{
        ...
        std::vector<vertex> &vertices,long numVertices,long offset,long targetIndex)
    {
        ...
        for (i=0; i<indexList.length(); i++)
        {
            ...
            for (j=1; j<poseNames.length() && !foundName; j+=2)
            {
                ...
                if (idx == i)
                {
                    // Doug Perkowski - 01/16/2009
                    // Make the name of the blendshape blendshapedeformername.blendshapename
                    // so we can preserve the deformer name.  Exporting to FBX requires the deformer
                    // name.
                    poseName = "";
                    if( params.useBlendshapeDeformerName )
                    {
                        poseName += getName().asChar();
                        poseName += ".";
                    }
        ...
        std::vector<vertex> &vertices,long numVertices,long offset,MString poseName)
    {
        ...
        // Doug Perkowski - 6/18/2012
        // if we don't export zero-vertex blendshapes, we need to keep track of them so
        // when we export blendshape animations we don't assume they are there and
        // try to refer to them by their index in Maya's arrays.
        if (p.offsets.size() == 0)
            std::cout << "Warning! Blendshape has 0 vertices.  Exporting it anyway so animation data is not corrupted.\n";
        m_poses.push_back(p);
        if (params.bsBB)
        {
