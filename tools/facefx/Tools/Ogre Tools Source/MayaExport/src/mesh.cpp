...
namespace OgreMayaExporter
{
    ...
    void Mesh::clear()
    {
        ...
        for (i=0; i<subMeshNonSharedBlendShapes.size(); i++)
        {
            if (subMeshNonSharedBlendShapes[i])
                delete subMeshNonSharedBlendShapes[i];
        }
        m_sharedGeom.vertices.clear();
        m_sharedGeom.dagMap.clear();
        m_vertexClips.clear();
        ...
        m_uvsets.clear();
        m_submeshes.clear();
        subMeshNonSharedBlendShapes.clear();
        if (m_pSkeleton)
            delete m_pSkeleton;
        m_pSkeleton = NULL;
    }
    ...
    MStatus Mesh::load(const MDagPath& meshDag,ParamList &params)
    {
        ...
        stat = cleanupData(meshDag, params);
        if (stat !=  MS::kSuccess)
        {
            std::cout <<  "Error cleaning up data for current mesh\n";
        }
    ...
    MStatus Mesh::loadBlendShapes(ParamList &params)
    {
        ...
        else
        {
            for (i=0; i<m_submeshes.size(); i++)
            {
                // Doug Perkowski - 6/8/11 
                // Added one to loadPoses's last parameter as instructed by createPose documentation.
                Submesh* pSubmesh = m_submeshes[i];
                if (pSubmesh->m_pBlendShape)
...
/******************** Methods to parse geometry data from Maya ************************/
    ...
    MStatus Mesh::getBlendShapeDeformer(const MDagPath &meshDag, OgreMayaExporter::ParamList &params)
    {
        ...
        if (params.exportBlendShapes)
        {
            ...
            {   
                MObject kObject = kDepNodeIt.item();
                // Doug Perkowski - 10/21/2008
                // Evolver content had 2 blend shape deformers and the existing code was always picking
                // Up the first deformer, even when it wasn't modifying the mesh.
                ...
                MFnBlendShapeDeformer MFnBlendShape(kObject);

                MObjectArray objects;
                MFnBlendShape.getBaseObjects ( objects ); 
                for(int i = 0; i < objects.length(); ++i)
                {
                    if(objects[i].hasFn(MFn::kMesh))
                    {
                        MFnMesh objectMeshFn(objects[i]);
                        if(objectMeshFn.fullPathName() == mesh.fullPathName())
                        {
                            pBlendShape = new BlendShape();
                            pBlendShape->load(kObject);
                            std::cout << "Found blend shape deformer " << pBlendShape->getName().asChar() << " for mesh " 
    ...
    // Set default values for data (rigidly skinned meshes, bone weights, position indices)
    MStatus Mesh::cleanupData(const MDagPath& meshDag,ParamList& params)
    {
        MFnDagNode meshTransformNode(meshDag.transform());
        MString meshName = meshTransformNode.name();
        ...
        if(m_pSkeleton)
        {
            joints = m_pSkeleton->getJoints();
        }
        int maxNumParents = 1;
        for(int i = 0; i < joints.size(); ++i)
        {
            MDagPath jointDag =	joints[i].jointDag;

            // Find out if this joint is a parent of the mesh.
            if(meshTransformNode.hasParent(jointDag.node()))
            {
                int parent = joints[i].parentIndex;
                int numParents = 0;
                
                // How far down in the skeleton is this joint?  We want the joint furthest down in the hierarchy.
                while(parent != -1)
                {
                    parent = joints[parent].parentIndex;
                    numParents++;
                }
                // If this joint is the furthest down in the hierarchy so far, rigidly skin the mesh to this bone.
        ...
        if( bRigidlySkinned )
        {
            ...
            {
                std::cout << "Warning: Unskinned mesh " << meshName.asChar() << " detected with no joints as parents. Mesh was skinned to the first joint.\n";
            }
            else 
            {
                std::cout << "Rigidly skinned mesh " << meshName.asChar() << " to joint " << joints[defaultJointIndex].name.asChar() << "\n";
            }
        }
        ...
        for(int i = 0; i < newvertices.size(); i++)
        {
            // The below 2 tests are possible in Max, never seen them in Maya, but they will crash Ogre.
            if(newvertices[i].pointIdx < 0)
            {
                newvertices[i].pointIdx = 0;
                assert(newpoints.length() != 0);
            }
            if(newvertices[i].normalIdx < 0)
            {
                newvertices[i].normalIdx = 0;
                assert(newnormals.length() != 0);
            }
            // Actually do the rigid skinning.
            if(defaultJointIndex!=-1)
            {
    ...
    MStatus Mesh::createSubmeshes(const MDagPath& meshDag,ParamList& params)
    {
        ...
        if (pBlendShape && !params.useSharedGeom)
        {   
            subMeshNonSharedBlendShapes.push_back(pBlendShape);
            pBlendShape = NULL;
        }
        return MS::kSuccess;
    }
...
/*********************************** Export mesh data **************************************/
    ...
    MStatus Mesh::writeOgreBinary(ParamList &params)
    {
        ...
        {
            // Doug Perkowski 3/17/2008 - Strip skeleton file paths from name
            std::string filename = StripToTopParent(params.skeletonFilename.asChar());
